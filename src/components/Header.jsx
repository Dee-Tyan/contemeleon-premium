      ...
      {navData.map((item) => {
        {
          return item.key === "create" ? (
            <Link
              key={item.id}
              href={item.link}
              className="rounded-md bg-pink-10 hover:bg-pink-10 hover:text-white text-center py-2 px-8 flex items-center justify-center font-bold leading-7"
            >
              {item.name}
            </Link>
          ) : (
            <div
              key={item.id}
              className="text-sm hover:text-pink-10 transition cursor-pointer font-bold"
            >
